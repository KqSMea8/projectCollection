...
const RecordEditorTable = React.createClass({
  propTypes: {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.params !== prevProps.params && (this.props.params.customerType === 'SHOP' || this.props.params.customerType === 'BRAND') && this.props.params.downloadType !== 'download') {
      this.refresh();
  ...
  render() {
    const {data, pagination, loading, visitPurposeMap} = this.state;
    ...
    const columns = [{
      ...
      title: (<div><p>{ this.props.type === 'SHOP' ? '拜访门店' : '拜访品牌'}</p><p style={{color: '#999'}}>{ this.props.type === 'SHOP' ? '门店ID' : '品牌ID'}</p></div>),
      width: 160,
      dataIndex: '',
      render(t, r) {
        return (<div>
      ...
      title: (<div><p>{(type === 'SHOP' && isPosSale === true) ? '代运营归属人' : '归属人'}</p><p style={{color: '#999'}}>{ (type === 'SHOP' && !isPosSale) ? 'BD／服务商／服务商员工' : ''}</p></div>),
      width: 120,
      dataIndex: '',
      render(t, r) {
        return (
      ...
      render: (t, r) => {
        ...
        return (<div>
          <div> {visitList}</div>
          { r.visitPurposeResult && <div>签约结果：{ResultMap[r.visitPurposeResult] }</div> }
      ...
      render(visitWay, index) {
        ...
        if (visitWayMap[visitWay] === '上门') {
          way = index.signStatus === '1' ? <RecordPopover data={index} type={type}/> : (<p style={{color: '#999'}} >未签到</p>);
        }
        return (
          <div>
    ...
    if (type !== 'SHOP') {
      columns.concat({
        title: (<div style={{textAlign: 'center'}}>商户巡检和培训</div>),
        width: 120,
...
export default RecordEditorTable;

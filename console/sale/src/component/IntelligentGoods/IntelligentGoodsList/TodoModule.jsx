...
class TodoListComponent extends React.Component {
  ...
  fetch(pageParams = {}) {
    ...
    ajax({
      // url: window.APP.crmhomeUrl + todoListRequestUrl[this.props.type],
      url: window.APP.kbservindustryprodUrl + '/item/leads/queryMerchantBySpStaffId.json',
      method: 'post',
      data: params,
      type: 'json',
      success: (result) => {
        ...
        if (result.status && result.status === 'succeed') {
          ...
          if ((!partnerId || !partnerName) && result.data && result.data.length) {
    ...
    return (<div>
        ...
        <div>
            <Spin spinning={loading}>
                <TodoList dataSource={this.state.data} checkTodoItem={this.checkTodoItem} env={this.props.env}/>
            </Spin>
        </div>
        <Row flex="end">

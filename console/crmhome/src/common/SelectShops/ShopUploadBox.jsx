...
import UploadBusinessFile from '../Upload/UploadBusinessFile';
...
const ShopUploadBox = React.createClass({
  ...
  */
  onProgress(fileNum, response = {}) {
    if (fileNum && response.logId) {
      // 有文件正在上传
      logId = response.logId;
      setTimeout(this.keepAjax, 900);
    } else {
      // 文件没有上传
      this.setState({ visibleProgress: false, finishTime: ''});
    }
  },
  cleanProgress() {
  ...
  keepAjax() {
    ...
    const legal = '/goods/itempromo/downloadRecruitShops.htm?planId=' + '&op_merchant_id=' + opMerchantId;
  ...
  render() {
    ...
    const opMerchantId = merchantIdInput ? merchantIdInput.value : '';
    return (<div>
      <Alert message="上传门店信息"
         description="请根据全部门店模板中的信息，按照本次参与活动的门店调整后上传。"
         type="info"
         showIcon
       />
      <div>
      <div {...this.props.form.getFieldProps('uploadStatu', {
        initialValue: {hasUpload: false, shopTotalCnt: 0},
      })}></div>
        <UploadBusinessFile
          beforeUpload={beforeUpload}
          disabled={false}
          data={{op_merchant_id: opMerchantId}}
          name="fileData"
          propsName="uploadBusinessFile"
          onProgress={this.onProgress}
          // action={'/goods/itempromo/uploadRecruitShops.json'}
          action={`${window.APP.ownUrl}/goods/itempromo/uploadPromoShops.json`}
          {...this.props}/>
      </div>
      <div>
        {
        ...
        {
          this.state.visibleProgress && <Progress style={{marginTop: '20px'}}data={this.state.data}/>
        }
        </div>
      </div>);
  },

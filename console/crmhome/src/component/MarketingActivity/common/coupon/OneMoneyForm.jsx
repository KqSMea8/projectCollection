...
import ReduceTypeFormItem from './ReduceTypeFormItem';
...
const OneMoneyForm = React.createClass({
  propTypes: {
    ...
    return (<Form horizontal form={this.props.form} className="kb-coupon-form">
      ...
      <FormItem
        ...
        <RadioGroup {...getFieldProps('myType', {
          onChange: this.onTypeChange,
          initialValue: 'ONE_MONEY',
        })}>
          {!isGenericIndustry && <Radio value="ONE_RATE">折扣券</Radio>}
      ...
      <FormItem
        ...
        required
        {...formItemLayout}>
        {/* isEdit ? <p className="ant-form-text">{data.itemName}</p> :*/
      ...
      {!isGenericIndustry && <FormItem
        label="更多商品详情："
        {...formItemLayout}>
        <Input placeholder="例如 https://www.alipay.com" {...getFieldProps('itemLink', {
          rules: [this.checkUrl],
        }) } />
      </FormItem>}
      <ReduceTypeFormItem form={this.props.form} data={data} isEdit={isEdit} isCampaignStart={isCampaignStart} />
      {!isGenericIndustry && <FormItem
        label="商品编码："
        required
      ...
      {!isGenericIndustry && <UseConditionFormItem form={this.props.form} data={data} isEdit={isEdit} isCampaignStart={isCampaignStart} />}
      <CommonForm form={this.props.form} data={data} isEdit={isEdit} isCampaignStart={isCampaignStart} />
    </Form>);
  },
});

export default OneMoneyForm;

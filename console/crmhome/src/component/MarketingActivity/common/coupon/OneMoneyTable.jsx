...
const OneMoneyTable = React.createClass({
  propTypes: {
  ...
  render() {
    ...
    let useCondtionString;
    if (data.customType === 'ONE_MONEY_REDUCETO') {
      useCondtionString = '同一商品满' + data.minConsumeNum + '件可享受优惠，且该商品最高优惠' + data.maxDiscountNum + '件';
    } else {
      if (data.useCondtion) {
        useCondtionString = '订单满' + data.useCondtion + '元可享用';
      } else {
        useCondtionString = '不限制';
      }
    }
    return (

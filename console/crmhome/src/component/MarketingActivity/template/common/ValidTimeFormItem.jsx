...
const ValidTimeFormItem = React.createClass({
  ...
  componentDidUpdate(prevProps) {
    ...
    if (initData !== prevProps.initData) {
      this.resetData(initData.templateNo);
    }
  },
  ...
  checkValidTimeType(rule, value, callback) {
    ...
    if (value === 'FIXED') {
      ...
      form.setFieldsValue({autoDelayFlag: ''});
    }
    callback();
  },
  ...
  render() {
    ...
    return (<FormItem
      ...
      <Select style={{width: 110, marginRight: 8}} placeholder="请选择" {...getFieldProps('validTimeType', {
        initialValue: (initData.bsnParams && initData.bsnParams.validTimeType) || 'RELATIVE',
        validateFirst: true,
      ...
      <div style={{display: validTimeType === 'RELATIVE' ? 'inline-block' : 'none', verticalAlign: 'bottom'}}>
        ...
        <InputNumber size="large" {...getFieldProps('validPeriod', {
          initialValue: initData.bsnParams && initData.bsnParams.validPeriod ? Number(initData.bsnParams.validPeriod) : '',
          validateFirst: true,
      ...
      <div style={{display: validTimeType === 'FIXED' ? 'inline-block' : 'none', verticalAlign: 'bottom'}}>
        <RangePicker {...getFieldProps('validTime', {
          initialValue: (initData.bsnParams && initData.bsnParams.validTimeFrom) ? [initData.bsnParams.validTimeFrom, initData.bsnParams.validTimeTo] : [],
          validateFirst: true,
          rules: [this.checkInputValue],

...
const UploadBusinessFile = React.createClass({
  ...
  render() {
    ...
    let actityType = document.getElementById('activiType');
    actityType = actityType ? actityType.value : undefined;

    const opMerchantId = merchantIdInput ? merchantIdInput.value : '';
    const ctoken = getCtokenFromCookie(window);
    /*eslint-disable */
    const legal = '/goods/itempromo/downloadPromoShops.htm?op_merchant_id=' + opMerchantId + '&ctoken=' + ctoken + '&promoTool=' + actityType;
    return (
      <FormItem
        label="文件上传"

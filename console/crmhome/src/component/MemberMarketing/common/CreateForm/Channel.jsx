...
const Channel = React.createClass({
  ...
  getInitialState() {
    ...
    if (initData.deliveryChannelImgUrl) {
      quanPicList = [{
        uid: '-1' + Math.random(),
        name: 'xxx.png',
        status: 'done',
    ...
    if (initData.deliveryChannels) {
      ...
      // 接收checkbox默认状态
      const assignCheckBox = assignInWith(checkBoxConfig, obj, (objValue, srcValue) => {
        ...
        <FormItem
...

          {
            checkBoxArr.map((item) => {
              return <div key={item.name}><Checkbox disabled={isDisabled} {...checkboxProps[item.name]} />{item.label}<br /></div>;
            })
          }
            ...
            <Row>
              <Col span="17" offset="7">
                <FormItem
                    ...
                    required
                    label="专属优惠图片："
                    help={isEdit && initData.deliveryChannelImgUrl ? '' : '建议：请尽量选择和券内容相关的图片，不超过2M。格式：bmp, png, jpeg, jpg, gif。建议尺寸: 500px＊500px'}
                >
                  {
                    isEdit && isDisabled ? (
                        <div>

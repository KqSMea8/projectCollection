...
import { retailersActivityType } from '../../config/AllStatus';
...
const ConsumeView = React.createClass({
  ...
  renderVoucherTable( item, itemKey ) {
    ...
    let discountType;
    if (this.props.discountForm.type === 'CONSUME_SEND') {
      discountType = '单品';
    } else {
      discountType = item.goodsIds && item.goodsIds.length > 0 ? '单品' : '全场';
    }
    ...
    return (
      <table className="kb-detail-table-6">
        ...
        <tr>
          ...
          <td>
            {discountType + retailersActivityType[item.itemDiscountType]}
          </td>
        ...
        </tr>
        {
          ...
          <tr>
            ...
            <td colSpan="3">{item.donateFlag ? '是' : '否'}</td>
          </tr> : <tr>
            <td className="kb-detail-table-label">券是否可以转赠</td>
            <td colSpan="5" >{item.donateFlag ? '是' : '否'}</td>
          </tr>
        }
        <tr>
  ...
  render() {
    ...
    let preCss = {fontFamily: '"Helvetica Neue", Helvetica, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", Arial, sans-serif', whiteSpace: 'normal', display: 'block', wordBreak: 'break-all'};

    if (discountForm.activityInstructions && discountForm.activityInstructions.indexOf('\n') > 0) {
      preCss = {fontFamily: '"Helvetica Neue", Helvetica, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", Arial, sans-serif', display: 'block', wordBreak: 'break-all'};
    }
    return (
        <div>
          ...
          <table className="kb-detail-table-6" style={{marginTop: 10}}>
            ...
            <tr>
              <td className="kb-detail-table-label">活动说明</td>
              <td colSpan="5"><pre style={preCss}>{ discountForm.activityInstructions ? discountForm.activityInstructions : '' }</pre></td>
            </tr>
            </tbody>
          </table>
        </div>

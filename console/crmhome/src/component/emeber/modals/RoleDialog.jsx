...
class RoleDialog extends React.Component {
  ...
  componentDidMount() {
    ...
    ajax({
      url: `/staff/queryPermissions.json?principalId=${roleId}${roleCode ? `&roleCode=${roleCode}` : ''}&principalType=ROLE&channel=CRMHOME`,
      method: 'get',
      type: 'json',
      success: (res) => {
    ...
    if (window.alipay && window.alipay.security && window.alipay.security.core) {
      this.setInputValue(document.querySelectorAll('.J_roleId'), roleId);
      window.light.ready(() => {
  ...
  dealResult(data) {
    const original = data.reduce((memo, item) => {
      const children = item.children.filter(child => !child.select) || [];
      if (!item.select || children.length) {
        memo.push({
          ...item,
          children,
        });

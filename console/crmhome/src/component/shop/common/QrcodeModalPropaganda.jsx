...
const QrcodeModalPropaganda = React.createClass({
  ...
  download() {
    location.href = this.appendParams(window.APP.crmhomeUrl + '/shop/downloadQRCode.htm');  // eslint-disable-line no-location-assign
  },
  ...
  render() {
    ...
    const imageSrc = this.appendParams(window.APP.crmhomeUrl + '/shop/qrCode.htm') + '&t=' + rand + forceRefreshParams;
    return (<div style={{textAlign: 'center', fontSize: 14, padding: 20, minHeight: 483}}>
      <Row style={{marginBottom: 20}}>
        <div><span style={{color: '#2db7f5'}}><Icon type="info-circle"/></span> 用户扫描二维码即可查看店铺详情</div>
      </Row>
      {activeKey === 'keyword' && <Row style={{margin: 20}}><QrcodeModalPropagandaKeywordInput onSubmit={this.onSubmitKeyword}/></Row>}
      <Row>

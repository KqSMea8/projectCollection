  ...
  render() {
    ...
    return (<Modal footer={null}
      ...
      <div className="shop-details-modal" style={{ height: 500, overflow: 'auto' }}>
        ...
        {repastData.descGroups && !!repastData.descGroups.length && (
          <div className="details-modal-desc">
            {repastData.descGroups.map((row, r) => {
              return (
                <div key={r}>
                  <div className="title">{row.title}</div>
                  {row.remarks && (row.remarks || []).map((col, i) => <div key={i}>{col}</div>)}
                </div>
              );
            })}
          </div>
        )}
        {repastData.services && !!repastData.services.length && (
          <div>
            <div className="detail-modal-title">
              <h3 className="kb-page-sub-title">服务流程</h3>
              <i />
            </div>
            <div>
              <Steps direction="vertical" size="small" current={1}>
                {repastData.services.map((item, i) => {
                  const title = <span><span style={{ color: '#666' }}>{item.name}</span>{`（${item.quantity}${item.unit}）`}</span>;
                  return (<Step key={i} title={title} status="wait" description={item.desc} />);
                })}
              </Steps>
            </div>
          </div>
